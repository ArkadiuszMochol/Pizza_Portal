...
public class RegistrationService {
    ...
    //metody wywoływania do rejestracji użytkownika -> zapis w tablicy i pliku users
    ...
    public void registration(TextField tfLogin, PasswordField pfPassword,
                             PasswordField pfPasswordConfirmation, Label lblInfo,
                             TextField lblResult, Label lblEquation) {
        try {
            if (isHuman(Integer.valueOf(lblResult.getText()), RegistrationController.result)) {
                ...
                RegistrationController.result = generatedRandomEquation(lblEquation);
                lblResult.clear();
            }else {
